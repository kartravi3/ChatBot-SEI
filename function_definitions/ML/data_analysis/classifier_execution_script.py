...
def text_classify(column_dict,column_name,train_df):
	...
	for idx,row in train_df.iterrows():
		print (column_name)
		print (train_df.at[idx,column_name])
...
def predict_test_data(column_dict,column_name,test_df):
	...
	try:
		...
		vectorizer_f.close()
		print ('4')
		#X_test_dtm=vect.transform(test_df['Question'].values.astype('U'))
		X_test_dtm=vect.transform(test_df['filtered_msg'])
	...
	try:
		for idx,row in test_df.iterrows():
			...
			if test_df.at[idx,''] in []:
				print ('Hello! Welcome to SEI. How can I help?')
	...
	for idx,row in test_df.iterrows():
		...
		#for getting the key from value
		key = list(column_dict)[list(column_dict.values()).index(y_pred_class[c])]
		#key = list(column_dict.keys())[list(column_dict.values()).index(y_pred_class[c])]
		#print key,y_pred_class[c],'yyyyy',c,'   ',y_pred_class
		#raw_input()
		#test_df.at[idx,'ML_'+column_name]=key
		print ("Question is ",test_df.at[idx,'Question'])
		print ('FINAL OUTPUT IS:',key)
		input()
